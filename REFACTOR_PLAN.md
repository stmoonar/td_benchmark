...
| 占位符 | 含义 | 当前值 |
|---|---|---|
| `{{BENCH_ROOT}}` | 旧 moe_bench 仓库根目录（只读参考） | `/data/cinnzhang_vllm_td_test/moe_bench_0607` |
| `{{NEW_ROOT}}` | **重构产出根目录（阶段一全部代码写在这里）** | `/data/cinnzhang_vllm_td_test/new_start` |
| `{{TD_FORK}}` | 我们修改过的 Triton-distributed fork | `/data/cinnzhang_vllm_td_test/triton_distributed-TD+Flux/python/triton_dist_fp8` |
| `{{TD_UPSTREAM}}` | 干净的上游 Triton-distributed（checkout 到 `1b9dc71a`）| `/data/cinnzhang_vllm_td_test/triton_distributed-TD+Flux/python/triton_dist` |
| `{{PYTHON}}` | 运行环境 python | `source /data/cinnzhang_vllm_td_test/venvs/vllm-td/bin/activate && python` |
